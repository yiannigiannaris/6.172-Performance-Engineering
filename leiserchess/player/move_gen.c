...
// void position_check(position_t* p) {
//   // display(p); 
//   for (fil_t r = 0; r < BOARD_WIDTH; r++) {
//     for (rnk_t c = 0; c < BOARD_WIDTH; c++) {
//       square_t currentSquare = square_of(c, r);
//       piece_t currentPiece = p->board[currentSquare];
//       int index = p->indexBoard[r][c];
//       ptype_t type = ptype_of(currentPiece);
//       if (type == PAWN || type == KING) {
//         location_t location = p->pieceLocations[index];
//         assert(index >= 0 && index < NUM_PIECES);
//         assert(location.row == r && location.col == c);
//         assert(location.isPiece);
//       } else {
//         assert(index == -1);
//       }
//     }
...
void remove_piece(position_t* p, square_t sq){
  piece_t piece = p->board[sq];
  int locInd = piece_loc_ind(piece);
  p->pieceLocations[color_of(piece)][locInd] = -1;
  // display(p);
}



void update_piececentric(position_t* p, move_t mv){
  square_t fromSquare = from_square(mv);
  square_t intSquare = intermediate_square(mv);
  square_t toSquare = to_square(mv);
  if (fromSquare != toSquare){
    piece_t fromPiece = p->board[fromSquare];
    if (fromSquare == intSquare){
      int locInd = piece_loc_ind(fromPiece);
      p->pieceLocations[color_of(fromPiece)][locInd] = toSquare;
    } else{
      piece_t intPiece = p->board[intSquare];
      int fromInd = piece_loc_ind(fromPiece);
      int intInd = piece_loc_ind(intPiece);
      p->pieceLocations[color_of(fromPiece)][fromInd] = toSquare;
      p->pieceLocations[color_of(intPiece)][intInd] = fromSquare;
    }
  }
  // display(p);
  // position_check(p);
...
fil_t fil_of(square_t sq) {
  ...
  fil_t f = ((sq/10) & FIL_MASK) - FIL_ORIGIN;
...
rnk_t rnk_of(square_t sq) {
  ...
  rnk_t r = ((sq%10) & RNK_MASK) - RNK_ORIGIN;
...
int generate_all(position_t* p, sortable_move_t* sortable_move_list,
  ...
  square_t* pieces_of_color = p->pieceLocations[color];  
  for (int i = 0; i < NUM_PIECES_SIDE; i++) {
    ...
    if (sq != -1) {
      ...
      // switch (typ) {
      // case EMPTY:
      //   printf("SHOULD NEVER GET HERE IN GEN: Location from pieceLocations was empty");
      //   break;
      // case PAWN:
      // case KING:
...
void display(position_t* p) {
  ...
  printf("WHITE pieceLocations: [");
  for (int i = 0; i < NUM_PIECES_SIDE; i++){
    square_t sq = p->pieceLocations[0][i];
    if (sq == -1){
      printf("(-1) ");
    }else{
  ...
  printf("]\n");
  printf("BLACK pieceLocations: [");
  for (int i = 0; i < NUM_PIECES_SIDE; i++){
    square_t sq = p->pieceLocations[1][i];
    if (sq == -1){
      printf("(-1) ");
    }else{
      printf("(%d,%d) ", rnk_of(sq), fil_of(sq));
  ...
  for (rnk_t r = 0; r < BOARD_WIDTH; r++) {
    for (fil_t f = 0; f < BOARD_WIDTH; f++) {
      ...
      if (color_of(p->board[sq]) == WHITE){
        if (ptype_of(p->board[sq]) == KING) {
          printf("KING:%d (%d,%d)  ", piece_loc_ind(p->board[sq]), r, f);
          continue;
        }

        if (ptype_of(p->board[sq]) == PAWN) {
          printf("PAWN:%d (%d,%d)  ", piece_loc_ind(p->board[sq]), r, f);
          continue;
        }
      }
    }
