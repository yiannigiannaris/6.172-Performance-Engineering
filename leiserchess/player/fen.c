...
static int parse_fen_board(position_t* p, char* fen) {
  ...
  while ((c = fen[c_count++]) != '\0') {
    ...
    case 'N':
      ...
      if (typ == KING){
        set_piece_index(&p->board[square_of(f, r)], 0);
      } else {
        set_piece_index(&p->board[square_of(f, r)], white_ind);
        white_ind++;
      }
      break;
    ...
    case 'n':
      ...
      if (typ == KING){
        ...
        set_piece_index(&p->board[square_of(f, r)], black_ind);
        black_ind++;
      }
      break;
...
int fen_to_pos(position_t* p, char* fen) {
  ...
  for (fil_t f = 0; f < BOARD_WIDTH; ++f) {
    for (rnk_t r = 0; r < BOARD_WIDTH; ++r) {
      ...
      if (typ == KING || typ == PAWN){
        p->pieceLocations[color_of(x)][piece_loc_ind(x)] = sq;
      }
    }
  }
  if (Kings[WHITE] == 0) {
