...
#define NUM_PIECES_SIDE 8
#define NUM_PIECES 2 * NUM_PIECES_SIDE
...
typedef struct position {
  ...
  square_t pieceLocations[2][NUM_PIECES_SIDE];
...
//NEW FUNCTIONS
static inline void set_piece_index(piece_t* x, uint8_t i){
  *x = ((i & IND_MASK) << IND_SHIFT) | (*x & ~(IND_MASK << IND_SHIFT));
}

static inline int piece_loc_ind(piece_t piece){
  return (piece >> IND_SHIFT) & IND_MASK;
}
